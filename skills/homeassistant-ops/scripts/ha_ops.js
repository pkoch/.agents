...
function sortListByKey(items, keyFields) {
  ...
  const keyOf = (item) => {
    if (!item || typeof item !== "object" || Array.isArray(item)) return [1, String(item)]
...
class ProgressReporter {
  ...
  #print() {
    ...
    let bar = "=".repeat(filled) + ">" + " ".repeat(Math.max(0, this.width - filled - 1))
...
class HAWebSocket {
  ...
  async connect() {
    ...
    this.ws.addEventListener("message", (ev) => {
      ...
      if (typeof id === "number" && this.pending.has(id)) {
        ...
        if (msg.success === false) {
          reject(new Error(`WS call failed: ${msg.type}: ${JSON.stringify(msg)}`))
        } else {
          resolve(msg.result)
        }
        return
    ...
    if (first?.type !== "auth_required") {
      throw new Error(`Unexpected initial WS message: ${JSON.stringify(first)}`)
    }
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      if (timeoutMs) {
        timeout = setTimeout(() => {
          ...
          wrapped.reject(new Error(`WS call timeout after ${timeoutMs}ms: ${messageType}`))
        }, timeoutMs)
      }
    })
  ...
  async recvJson({ timeoutMs } = {}) {
    ...
    return await Promise.race([
      promise,
      new Promise((_, reject) => setTimeout(() => reject(new Error("WS recv timeout")), timeoutMs)),
    ])
  }
}
...
function printTopHelp() {
  ...
  console.log("  snapshot                Create a JSON snapshot for diff/rollback")
  console.log("  rollback                Rollback registry changes from a snapshot")
  console.log("  find-references          Find entity_id/string references")
  console.log("  traces                   List/view automation or script traces")
  console.log("  tail-events              Tail Home Assistant events over WebSocket")
  console.log("  name-review-from-backup  Offline naming analysis from a backup")
  console.log("")
...
function printCleanupHelp() {
  ...
  console.log("  --pattern <cat:regex>      Pattern for prefix-generic (repeatable)")
  console.log("  --blueprint-pattern <re>   Regex to match blueprint paths for create-groups step")
  console.log("  --log <path>               Log path (default: ha_cleanup_<timestamp>.md)")
  console.log("  --apply                    Apply changes (required to mutate HA)")
  console.log("  --json                     Output proposed changes as JSON (implies dry-run)")
...
async function cmdCleanup(argv) {
  ...
  for (const step of steps) {
    if (!AVAILABLE_CLEANUP_STEPS.includes(step)) {
      die(`Unknown step: ${step}\nAvailable: ${AVAILABLE_CLEANUP_STEPS.join(", ")}`, 2)
    }
  }
  ...
  const logPath = values.log ? String(values.log) : `ha_cleanup_${timestamp}.md`
...
async function cleanupStepRenameSwitchSuffix(ctx) {
  ...
  const progress = new ProgressReporter(candidates.length, "Renaming switch suffixes: ")
  for (const [entityId, friendly] of candidates) {
    ...
    await cleanupRenameEntity(ctx, entityId, friendly, newName, "switch-suffix")
    progress.update()
  }
  progress.finish()
}
...
function pickMainSwitch(switches) {
  ...
  candidates.sort((a, b) => b[0] - a[0] || a[1] - b[1] || (a[2] < b[2] ? -1 : 1))
...
function deriveGroupEntityId(switches, automationEntityId, targetAreaId, switchAreaId) {
  ...
  if (targetAreaId) {
    const inArea = [...switches].filter((s) => switchAreaId[s] === targetAreaId).sort()
  ...
  else if (objectId.endsWith("_switches")) objectId = objectId.slice(0, -"_switches".length)
  return `switch.${objectId}_group`
}

async function cleanupStepCreateGroups(ctx, blueprintPattern) {
  const bpRe = blueprintPattern ? new RegExp(String(blueprintPattern)) : /sync.*switch/i
  ...
  for (const st of ctx.states) {
    ...
    let cfg
    try {
      cfg = await ctx.rest.getJson(`/api/config/automation/config/${automationId}`)
    } catch (e) {
      ctx.warnings.push(`Failed to load config for ${entityId}: ${e?.message || e}`)
      continue
    }
    ...
    switches = switches.filter((s) => typeof s === "string" && s.startsWith("switch."))
  ...
  const progress = new ProgressReporter(candidates.length, "Creating/updating groups: ")
  ...
  for (const [automationEntityId, alias, parsed, switches] of candidates) {
    ...
    const targetAreaId = ctx.normalizedAreaToId[normalizeAreaName(areaName)] || null
    const displayAreaName = targetAreaId ? ctx.areaIdToName[targetAreaId] || areaName : areaName
    ...
    if (!knownEntityIds.has(desiredGroupId) && knownEntityIds.has(candidateAll)) {
      const existing = ctx.entityEntryById[candidateAll]
      if (existing && typeof existing === "object" && existing.platform === "group") {
        try {
          ...
          let nextEntry = updated && typeof updated === "object" ? updated : null
          if (!nextEntry) {
            try {
          ...
          ctx.warnings.push(`Failed to rename ${candidateAll}: ${e?.message || e}`)
        }
      }
    }
    ...
    if (knownEntityIds.has(desiredGroupId)) {
      ...
      try {
        ...
        if (!groupEntry || typeof groupEntry !== "object" || groupEntry.platform !== "group") {
          ctx.warnings.push(`${desiredGroupId} exists but is not a group helper`)
          progress.update()
          continue
        }
        ...
        const optionsFlow = await ctx.rest.postJson("/api/config/config_entries/options/flow", {
          handler: configEntryId,
        })
        const flowId = getStr(optionsFlow, "flow_id")
        if (flowId) {
          await ctx.rest.postJson(`/api/config/config_entries/options/flow/${flowId}`, {
            entities: switches,
            hide_members: true,
            all: false,
          })
        ...
        const updated = await ctx.ws.call("config/entity_registry/update", payload)
        if (updated && typeof updated === "object") ctx.entityEntryById[desiredGroupId] = updated
        action = action || "updated"
      } catch (e) {
        ctx.warnings.push(`Failed to update group ${desiredGroupId}: ${e?.message || e}`)
        progress.update()
        continue
      }
    } else {
      ...
      try {
        const flowStart = await ctx.rest.postJson("/api/config/config_entries/flow", {
          handler: "group",
        })
        const flowId = getStr(flowStart, "flow_id")
        if (!flowId) throw new Error(`Unexpected flow start: ${JSON.stringify(flowStart)}`)
        ...
        ctx.warnings.push(`Failed to create group ${desiredGroupId}: ${e?.message || e}`)
        progress.update()
        continue
      }
    }
...
async function cleanupStepPrefixLightsCove(ctx) {
  ...
  for (const st of ctx.states) {
    ...
    if (!(entityId.startsWith("switch.") || entityId.startsWith("light."))) continue
    const attrs = getDict(st, "attributes") || {}
    const friendly = getStr(attrs, "friendly_name")
    if (friendly && re.test(friendly)) candidates.push([entityId, friendly])
  }
  if (!candidates.length) return
  const progress = new ProgressReporter(candidates.length, "Prefixing Lights/Cove names: ")
  for (const [entityId, friendly] of candidates) {
    ...
    await cleanupRenameEntity(ctx, entityId, friendly, newName, "lights-cove-prefix")
    progress.update()
  }
  progress.finish()
}
...
async function cleanupStepPrefixGeneric(ctx, patterns) {
  ...
  if (!candidates.length) return
  const progress = new ProgressReporter(candidates.length, "Prefixing generic patterns: ")
  for (const [entityId, friendly, category] of candidates) {
    const areaName = ctx.resolveAreaName(entityId)
    if (!areaName) {
      ctx.warnings.push(`${entityId}: cannot resolve area for ${category} prefix`)
      progress.update()
      continue
    }
    if (friendly.startsWith(areaName + " ")) {
...
function cleanupWriteLog(logPath, baseUrl, timestamp, ctx, steps, dryRun) {
  ...
  if (byCategory.size) {
    ...
    for (const cat of cats) {
      lines.push("", `### ${cat}`)
      const rs = [...byCategory.get(cat)].sort((a, b) => (a.entityId < b.entityId ? -1 : 1))
      for (const r of rs)
        lines.push(
...
function printSnapshotHelp() {
  ...
  console.log("")
  console.log("Options:")
  console.log("  --out <path>         Output JSON path (default: ha_snapshot_<timestamp>.json)")
  ...
  console.log("  --no-lovelace        Skip Lovelace dashboard config snapshots")
  console.log("  -h, --help           Show this help")
}
...
async function cmdSnapshot(argv) {
  ...
  const timestamp = utcTimestamp()
  const outPath = values.out ? String(values.out) : `ha_snapshot_${timestamp}.json`
  const indent = Number(values.indent)
  if (!Number.isFinite(indent) || indent < 0) die(`Invalid --indent: ${values.indent}`, 2)
  ...
  try {
    try {
      ...
      snapshot.warnings.push(`Failed to load area registry: ${e?.message || e}`)
    }
    ...
    try {
      ...
      snapshot.warnings.push(`Failed to load device registry: ${e?.message || e}`)
    }
    ...
    try {
      ...
      snapshot.warnings.push(`Failed to load entity registry: ${e?.message || e}`)
    }
  } finally {
    await ws.close()
  }
  ...
  if (!values["no-automations"]) {
    ...
    for (const st of states) {
      ...
      const cfg = await rest
        ...
        .catch(() => null)
      if (cfg && typeof cfg === "object") automations[entityId] = { id: automationId, config: cfg }
    }
  ...
  if (!values["no-scripts"]) {
    ...
    for (const st of states) {
      ...
      const cfg = await rest
        ...
        .catch(() => null)
      if (cfg && typeof cfg === "object") scripts[entityId] = { id: scriptId, config: cfg }
    }
  ...
  if (!values["no-scenes"]) {
    ...
    for (const st of states) {
      ...
      const cfg = await rest
        ...
        .catch(() => null)
      if (cfg && typeof cfg === "object") scenes[entityId] = { id: sceneId, config: cfg }
    }
    ...
    if (Array.isArray(dashboards)) {
      for (const d of dashboards) {
        ...
        const cfg = await rest
          ...
          .catch(() => null)
        if (cfg && typeof cfg === "object") lovelace[dashId] = { dashboard: d, config: cfg }
      }
    } else {
      const cfg = await rest
      ...
      if (cfg && typeof cfg === "object") {
        ...
        snapshot.warnings.push("Lovelace API not available or not storage-based dashboards.")
      }
    }
  ...
  if (snapshot.warnings.length) console.log(`warnings: ${snapshot.warnings.length}`)
  return 0
}
...
function printRollbackHelp() {
  console.log("Usage: node scripts/ha_ops.js rollback <snapshot.json> [options]")
  console.log("")
  console.log("Options:")
  console.log("  --dry-run       Show what would be changed without applying")
  console.log("  --yes, -y       Apply changes without confirmation")
  console.log("  --log <path>    Log file path (default: ha_rollback_<timestamp>.md)")
  console.log("  -h, --help      Show this help")
}
...
async function cmdRollback(argv) {
  ...
  if (!fs.existsSync(snapshotPath)) die(`Snapshot file not found: ${snapshotPath}`, 2)
  ...
  try {
    ...
    for (const snap of snapshotEntities) {
      ...
      const cur = kind === "unique_id" ? currentByUniqueId.get(value) : currentByEntityId.get(value)
      if (!cur) {
        if (kind === "unique_id") {
          warnings.push(
  ...
  for (const rb of [...rollbacks].sort((a, b) => (a.targetEntityId < b.targetEntityId ? -1 : 1))) {
    let header = `- ${rb.currentEntityId}`
    if (rb.currentEntityId !== rb.targetEntityId) header += ` -> ${rb.targetEntityId}`
    if (rb.uniqueId) header += ` (unique_id=${JSON.stringify(rb.uniqueId)})`
    console.log(header)
    for (const [k, v] of Object.entries(rb.fields).sort(([a], [b]) => (a < b ? -1 : 1))) {
  ...
  const rbKey = (rb) => `${rb.uniqueId || ""}||${rb.currentEntityId}||${rb.targetEntityId}`
  ...
  try {
    const renames = rollbacks.filter((r) => r.currentEntityId !== r.targetEntityId)
    const updates = rollbacks.filter((r) => Object.keys(r.fields).length)

    if (renames.length) {
      const progress = new ProgressReporter(renames.length, "Restoring entity_ids: ")
    ...
    if (updates.length) {
      const progress = new ProgressReporter(updates.length, "Restoring registry fields: ")
      for (const rb of updates) {
        const entityIdForUpdate = renamedOk.has(rbKey(rb)) ? rb.targetEntityId : rb.currentEntityId
        try {
          await ws2.call("config/entity_registry/update", {
          ...
          applyWarnings.push(`Failed to update ${entityIdForUpdate}: ${e?.message || e}`)
        }
        progress.update()
      }
      progress.finish()
  ...
  const timestamp = utcTimestamp()
  const logPath = values.log ? String(values.log) : `ha_rollback_${timestamp}.md`
  const lines = [
...
function printFindReferencesHelp() {
  ...
  console.log("  --needles-file <path> File with one needle per line (# comments allowed)")
  console.log("  --map-json <path>     JSON mapping old->new (searches old keys)")
  console.log("  --backup-root <path>  Path to a Home Assistant backup root (optional)")
  console.log("  --max-bytes <n>       Max file size to scan in backups (default: 5000000)")
  console.log("  --out <path>          Output markdown path (default: ha_refs_<timestamp>.md)")
...
function compileUnionRegex(needles) {
  const escaped = needles.filter(Boolean).map((n) => n.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))
...
function loadNeedles(values) {
  ...
  if (values["map-json"]) {
    const mapping = JSON.parse(fs.readFileSync(String(values["map-json"]), "utf-8"))
...
async function findLiveHits(rest, needleRe) {
  ...
  for (const st of states) {
    ...
    const cfg = await rest
      ...
      .catch(() => null)
    if (cfg && typeof cfg === "object") scanConfig("script", entityId, scriptId, cfg)
  }
  ...
  for (const st of states) {
    ...
    const cfg = await rest
      ...
      .catch(() => null)
    if (cfg && typeof cfg === "object") scanConfig("scene", entityId, sceneId, cfg)
  }
  ...
  if (Array.isArray(dashboards)) {
    for (const d of dashboards) {
      ...
      const cfg = await rest
        ...
        .catch(() => null)
      if (cfg && typeof cfg === "object") scanConfig("lovelace", dashId, null, cfg)
    }
  } else {
    const cfg = await rest
      .getJson("/api/lovelace/config", { expectedStatuses: [200, 404] })
      .catch(() => null)
    if (cfg && typeof cfg === "object") scanConfig("lovelace", "default", null, cfg)
    else warnings.push("Lovelace API not available or not storage-based dashboards.")
  }
...
function writeRefsReport(outPath, needles, liveHits, liveWarnings, backupHits, backupWarnings) {
  const lines = [
  ...
  if (backupHits.length) {
    ...
    for (const hit of sorted) lines.push(`- \`${hit.path}\`:${hit.line}: matched \`${hit.needle}\``)
  } else {
    lines.push("- (none)")
  }
  lines.push("")
...
async function cmdFindReferences(argv) {
  ...
  if (!needles.length) die("No needles. Use --needle, --needles-file, or --map-json.", 2)
  ...
  if (!values["no-backup"] && values["backup-root"]) {
    ...
    try {
      const [hits, warnings] = findBackupHits(String(values["backup-root"]), needleRe, maxBytes)
  ...
  writeRefsReport(outPath, needles, liveHits, liveWarnings, backupHits, backupWarnings)
  if (values["json-out"]) {
    writeRefsJsonReport(
...
function printTailEventsHelp() {
  ...
  console.log("  --device-id <id>     Device registry ID to include (repeatable)")
  console.log("  --seconds <n>        Stop after N seconds (0 = run until interrupted)")
...
function printTracesHelp() {
  ...
  console.log("  --entity-id <entity_id>  Filter by entity_id (e.g., automation.my_automation)")
  console.log("  --run-id <run_id>        Get detailed trace for a specific run")
  ...
  console.log("  node scripts/ha_ops.js traces --entity-id automation.doorbell_announce")
  console.log("")
  console.log("  # Get detailed trace for a specific run")
  console.log("  node scripts/ha_ops.js traces --item-id 1757182154251 --run-id abc123...")
}
...
async function cmdTraces(argv) {
  ...
  if (entityId && !itemId) {
    ...
    if (!resolvedId) {
      die(`Entity ${entityId} does not have an 'id' attribute (required for trace lookup)`, 2)
    }
...
async function cmdTailEvents(argv) {
  ...
  if (!Number.isFinite(seconds) || seconds < 0) die(`Invalid --seconds: ${values.seconds}`, 2)

  const eventTypes = (values["event-type"].length ? values["event-type"] : ["state_changed"])
    .map(String)
    .filter(Boolean)
  ...
  try {
    ...
    console.log(`[${utcNowIso()}] Subscribed to ${JSON.stringify(uniqueEventTypes)}`)
    ...
    while (true) {
      ...
      if (entityFilter.size || deviceIeeeFilter.size || deviceIdFilter.size) {
        ...
        if (deviceIeeeFilter.size) {
          ...
          matched = matched || [...ieees].some((id) => deviceIeeeFilter.has(id))
        }
        if (deviceIdFilter.size) {
      ...
      if (etype === "state_changed") {
        ...
        const ctx = newState && typeof newState === "object" ? newState.context : null
        console.log(
        ...
        let targets = []
        if (serviceData && typeof serviceData === "object") targets = asList(serviceData.entity_id)
        console.log(
...
function printNameReviewHelp() {
  ...
  console.log("")
  console.log("Options:")
  console.log("  --backup-root <path>       Path to the Home Assistant backup root (required)")
  console.log(
    "  --out <path>               Output markdown path (default: ha_name_review_<timestamp>.md)",
  )
  console.log("  --limit <n>                Max candidates (default: 300)")
  console.log("  --include-diagnostic       Include diagnostic/config entities")
  console.log("  --include-lights-cove      Include entities mentioning 'Lights' or 'Cove'")
  console.log("  -h, --help                 Show this help")
}
...
function entityDisplayName(entity, deviceName) {
  ...
  if (entity.has_entity_name && deviceName && originalName) return `${deviceName} ${originalName}`
...
function loadRestoreStateFriendlyNames(storageDir) {
  ...
  for (const item of payload.data) {
    ...
    const name = attrs.friendly_name
    if (entityId && typeof name === "string" && name.trim()) friendly[entityId] = name.trim()
  }
  return friendly
}
...
async function cmdNameReviewFromBackup(argv) {
  ...
  for (const a of areaRegistry)
    if (a && typeof a === "object" && typeof a.id === "string") areasById[a.id] = a
  const devicesById = {}
  for (const d of deviceRegistry)
    if (d && typeof d === "object" && typeof d.id === "string") devicesById[d.id] = d
  ...
  const resolveAreaId = (entity) => {
    if (typeof entity.area_id === "string" && entity.area_id) return entity.area_id
  ...
  for (const entity of entityRegistry) {
    ...
    const currentName = restoreFriendlyNames[entityId] || entityDisplayName(entity, deviceName)
  ...
  for (const [entity, areaId, _deviceName, currentName] of resolved) {
    ...
    if (!values["include-lights-cove"] && LIGHTS_COVE_NAME_PATTERN.test(currentName)) continue
    ...
    const areaName = typeof area.name === "string" && area.name ? area.name : areaId
    ...
    const disabledBy = typeof entity.disabled_by === "string" ? entity.disabled_by : null
    const hiddenBy = typeof entity.hidden_by === "string" ? entity.hidden_by : null
  ...
  const limit = Number(values.limit)
  if (!Number.isFinite(limit) || limit < 0) die(`Invalid --limit: ${values.limit}`, 2)

  const timestamp = utcTimestamp()
  const outPath = values.out ? String(values.out) : `ha_name_review_${timestamp}.md`
