...
function renderTaskList(theme: any, tasks: TodoTask[], expanded: boolean): string {
  ...
  if (!expanded && tasks.length > max) {
    lines.push(
      theme.fg("dim", `… ${tasks.length - max} more (${keyHint("expandTools", "to expand")})`),
    )
  }
  return lines.join("\n")
}
...
export default function todosExtension(pi: ExtensionAPI) {
        ...
        if (details.comments) {
          if (expanded || details.comments.length <= 3) {
            ...
            lines.push(
              theme.fg(
                "dim",
                `${details.comments.length} comments (${keyHint("expandTools", "to show")})`,
              ),
            )
          }
        }
  ...
  const commandDescription = "Todoist tasks: /todo <active|pending|completed|all|sync|setup|doctor>"
